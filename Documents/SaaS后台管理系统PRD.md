...
| SaaS管理 | 客户管理 / 门店管理 | 管理门店基础信息、版本、功能授权、应用授权、支付配置和状态。 |
...
说明：独立的“门店权益管理”已从 SaaS 产品管理中移除，其能力并入“客户管理 / 门店管理 / 功能授权 / 门店应用授权”。
...
设备品牌不作为单独页面展示，入口放在设备列表搜索栏按钮区。

设备品牌字段：品牌名称、品牌编码、品牌状态、设备数量、更新时间。  
设备品牌动作：新增品牌、编辑品牌、禁用品牌。
...
| 查询 | 根据筛选条件刷新下属门店列表。 |
| 重置 | 清空筛选条件并恢复列表。 |
| 维护 | 打开维护弹窗，调整资金入账方、服务费承担方和会员体系归属。 |
...
4. 删除、停用、关闭、下线等风险操作应使用确认文案说明影响范围。
5. 状态字段必须使用不同颜色标签区分。
6. 同一状态下不能同时展示互斥动作，例如：
   - 应用上架和下架不能同时出现。
   - 应用启用和停用不能同时出现。
   - 顶部公告启用和停用不能同时出现。
   - 公告发布和下线不能同时出现。
7. 当前为前端原型，操作成功后使用本地状态即时刷新并显示成功提示。
8. 每次调整前端原型的信息架构、字段、动作或业务逻辑时，必须同步更新本 PRD。
...
6. 左侧菜单存在销售渠道、客户管理、财务中心、统计、设置和 SaaS管理模块。
...
14. 设置菜单位于 SaaS管理菜单前面。
15. 设置下包含门店设置和门店管理。
16. 门店设置用于维护当前门店资料，门店管理用于集团维护下属门店资金和会员体系归属配置。
17. SaaS管理下包含客户管理、SaaS产品管理、平台资源管理、公告管理、平台通用能力和 SaaS用户管理。
18. 独立的门店权益管理不再出现。
19. 门店应用授权入口在门店管理的功能授权中展示。
...
2. 功能授权抽屉展示授权版本、授权到期、授权状态、版本基础功能。
3. 功能授权抽屉展示门店应用授权列表。
...
5. 维护弹窗支持调整资金入账方、服务费承担方和会员体系归属，选项均为集团或门店。
...
1. 设备、设备品牌、打印模板页面能展示列表、筛选、详情或编辑动作。
2. 日志类页面能展示操作记录和详情。
3. 用户管理类页面能展示员工、权限、部门的新增、编辑、禁用或权限配置动作。
